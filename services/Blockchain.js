...
class Blockchain {
    ...
    async _addBlock(block) {
        ...
        return new Promise(async (resolve, reject) => {
           if (self.height > 0)
           {
            let previousBlock = await self.getBlockByHeight(self.height - 1);
    ...
    async getStarsByWalletAddress (address) {
        ...
        return new Promise((resolve, reject) => {
             self.chain.forEach(async (block) => {
                ...
                block.body = await block.getBData();
                if (block.height !== 0 && block.body !== null && block.body.data.owner === address) {
                    
                    stars.push(block.body.data);
                }
            });
            resolve(stars);
